...
import { addTask, completeCurrentStep, defaultSettings, defaultStats, deleteAllData, deleteCheckinHistory, deleteCompletedTasks, exportAllData, loadData, migrateTask, recordStepCompleted, recordStrategyUse, reflectionTime, saveCheckin, saveReflection, saveSettings, setCurrentStep, toggleTask, updateTask } from './store';
...
describe('settings (local demo mode)', () => {
  ...
  it('persists a valid restart point as part of settings', async () => {
    const restartMemory = {
      taskId: 'task-1',
    ...
    expect(settings.restartMemory).toEqual(restartMemory);
  });

  it('drops malformed restart memory instead of letting it break Today', async () => {
    localStorage.setItem(
      'nuvora-demo-data-v1',
      JSON.stringify({
        tasks: [],
    ...
    expect(settings.restartMemory).toBeNull();
...
describe('resilience to corrupted or invalid stored data', () => {
  ...
  it('can still save when stored data is missing its lists (e.g. older or partly written data)', async () => {
    localStorage.setItem('nuvora-demo-data-v1', JSON.stringify({ settings: {} }));
    await saveCheckin('demo', { answers: { mood: 2 }, risk: null });
    await saveReflection('demo', { manageable: 'Still saves', hard: '' });
    const saved = await addTask('demo', { title: 'Still adds', module: 'Other', due: '', bucket: 'today' });
    const { tasks, checkins, reflections } = await loadData('demo');
    expect(checkins).toHaveLength(1);
    expect(reflections[0].manageable).toBe('Still saves');
    expect(tasks.map(t => t.id)).toEqual([saved.id]);
  });
...
describe('saveReflection', () => {
  ...
  it('returns the saved entry with an id and createdAt, ready to put into state', async () => {
    const saved = await saveReflection('demo', { manageable: 'Returned entry', hard: '' });
    expect(saved).toMatchObject({ manageable: 'Returned entry', id: expect.any(String) });
    expect(Number.isNaN(Date.parse(saved.createdAt))).toBe(false);
    const { reflections } = await loadData('demo');
    expect(reflections[0]).toEqual(saved);
  });

  it('loads reflections newest first whatever shape createdAt was stored in, without rewriting them', async () => {
    const stored = [
      { id: 'a', manageable: 'Oldest', createdAt: '2026-01-01T10:00:00.000Z' },
      { id: 'b', text: 'Seeded shape', createdAt: { seconds: Date.parse('2026-03-01T10:00:00Z') / 1000, nanoseconds: 0 } },
      { id: 'c', manageable: 'Newest', createdAt: '2026-04-01T10:00:00.000Z' },
      { id: 'd', manageable: 'No date' },
    ];
    localStorage.setItem('nuvora-demo-data-v1', JSON.stringify({ tasks: [], checkins: [], reflections: stored, settings: {} }));
    const { reflections } = await loadData('demo');
    expect(reflections.map(r => r.id)).toEqual(['c', 'b', 'a', 'd']);
    expect(JSON.parse(localStorage.getItem('nuvora-demo-data-v1')).reflections).toEqual(stored);
  });

  it('reflectionTime reads ISO strings, Firestore Timestamps and {seconds} objects consistently', () => {
    const ms = Date.parse('2026-09-24T09:00:00.000Z');
    expect(reflectionTime({ createdAt: new Date(ms).toISOString() })).toBe(ms);
    expect(reflectionTime({ createdAt: { toMillis: () => ms, seconds: ms / 1000 } })).toBe(ms);
    expect(reflectionTime({ createdAt: { seconds: ms / 1000, nanoseconds: 0 } })).toBe(ms);
    expect(reflectionTime({ createdAt: 'not a date' })).toBe(0);
    expect(reflectionTime({})).toBe(0);
  });
