...
import { addTask, completeCurrentStep, defaultSettings, loadData, migrateTask, saveSettings, setCurrentStep, toggleTask, updateTask } from './store';
...
describe('settings (local demo mode)', () => {
  ...
  it('saves and reloads settings', async () => {
    ...
    const { settings } = await loadData('demo');
    expect(settings).toEqual({ calmMode: true, reducedMotion: true, textScale: 1.15, displayName: '' });
  });
