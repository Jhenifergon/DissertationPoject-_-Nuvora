...
// The switch between Firebase mode and demo mode. NEXT_PUBLIC_ variables are
// written into the JavaScript when the app is built, so the mode is fixed
// per build: a build made with .env.local filled in (including the one
// copied into the Android and iOS apps) always uses Firebase, and a build
// without it always runs the local demo. The Firebase web config is not a
// secret — access to data is controlled by Firebase Auth and firestore.rules.
