...
describe('support-summary copying', () => {
  ...
  it('shows an accessible error status when the clipboard write fails, without crashing', async () => {
    ...
    const status = await screen.findByText(
      "We couldn't copy that automatically. You can select and copy the text above instead."
    );
  ...
  it('disables the copy button until a check-in has been completed', async () => {
    ...
    expect(
      screen.getByText(
        'Complete a check-in to prepare a short support summary.'
      )
    ).toBeInTheDocument();
  });
});
...
describe(
  'live status messages carry the correct ARIA role',
  () => {
...
describe('offline banner', () => {
  ...
  it('shows when the browser goes offline, and hides again when back online', async () => {
    ...
    await screen.findByText(
      /You’re offline\. Some changes may take a little longer to sync\./
    );
...
describe(
  'navigation: every screen reachable without the bottom nav must have a way back',
  () => {
    ...
    for (const {
      ...
      it(`${openVia}: has a working way back to Today`, async () => {
        ...
        fireEvent.click(
          await screen.findByText(openVia)
        );
...
describe(
  'Calm Mode extends beyond Today (Tasks, Learn, Progress)',
  () => {
    ...
    it("Tasks: hides the week/later tabs and each task's step preview by default, both reachable via a link", async () => {
      ...
      await screen.findByText(
        'Calm Mode is on — only one thing is shown at a time.'
      );
    ...
    it('Progress: collapses strategies and trend history behind one link', async () => {
      ...
      expect(
        screen.queryByText(
          'Recent pressure patterns'
        )
      ).not.toBeInTheDocument();

      fireEvent.click(
        screen.getByText(
          'Show trends & strategies'
        )
      );
...
describe(
  ...
  () => {
    it('shows a Patterns panel once there is enough check-in history to say something meaningful', async () => {
      ...
      expect(
        screen.getByText(
          /Task initiation has been your largest contributor/
        )
      ).toBeInTheDocument();
    });
...
describe(
  'barrier-history-aware Overwhelmed Mode ordering',
  () => {
...
describe(
  'Overwhelmed Mode barrier panels',
  () => {
...
describe(
  "Small resets are tied to the student's actual current task",
  () => {
    function seedTask() {
      localStorage.setItem(
        'nuvora-demo-data-v1',
    ...
    it('references the real task title instead of a generic example', async () => {
      ...
      expect(
        screen.getByText(
          /Don't finish "Draft chapter 2"/
        )
      ).toBeInTheDocument();

      expect(
        screen.getByText(
          /Turn "Draft chapter 2" into/
        )
      ).toBeInTheDocument();

      expect(
        screen.getByText(
          /The low-energy version of "Draft chapter 2"/
        )
      ).toBeInTheDocument();
      ...
      expect(
        screen.getByRole('button', {
          name:
            /Open .Draft chapter 2. in my plan/,
        })
      ).toBeInTheDocument();
    });
    ...
    it('falls back to the original generic wording when there is no open task', async () => {
      ...
      expect(
        screen.getByText(
          "Don't finish the task. Open it and identify only the first action."
        )
      ).toBeInTheDocument();
